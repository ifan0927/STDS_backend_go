...
func TestPublishDispatchesTypedEventToSubscribers(t *testing.T) {
	t.Run("dispatches typed event to subscribers", func(t *testing.T) {
		bus := New()
		called := false
	...
	t.Run("calls subscribers in registration order", func(t *testing.T) {
		bus := New()
		order := make([]int, 0, 2)
	...
	t.Run("returns subscriber error", func(t *testing.T) {
		bus := New()
	...
	t.Run("ignores events without subscribers", func(t *testing.T) {
		bus := New()
