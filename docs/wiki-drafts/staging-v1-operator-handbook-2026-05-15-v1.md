...
| `stds-github-deploy-staging@stds-439609.iam.gserviceaccount.com` | Backend GitHub deploy service account | Submits Cloud Build, uses the project Cloud Build source bucket, can act as `stds-cloud-build-staging`, and needs narrow Cloud Run service read/traffic-update permission (`run.services.get` / `run.services.update`) for pre-deploy traffic capture and smoke rollback. The rollback drill also showed it needs Artifact Registry read on the staging backend repo and act-as on the runtime service account for `gcloud run services update-traffic` to exit cleanly. |
...
| GitHub Actions fails at `Capture pre-deploy Cloud Run traffic` with `run.services.get` | The failure is before Cloud Build deploy. The GitHub deploy SA is trying to read the current Cloud Run service traffic split for rollback evidence, not deploy the image itself. Run 25962307601 showed this for `stds-github-deploy-staging@stds-439609.iam.gserviceaccount.com`. | Grant the backend GitHub deploy SA narrow Cloud Run service read and traffic update permissions (`run.services.get` / `run.services.update`) for the staging service, then rerun from the same intended ref. Record the run ID and IAM binding summary without secrets. |
| Rollback drill reaches `update-traffic` but exits non-zero with Artifact Registry or act-as errors | `gcloud run services update-traffic` validates deployed service state. Run 25963082591 first exposed missing `artifactregistry.repositories.downloadArtifacts`, then missing `iam.serviceAccounts.actAs` on the runtime service account. In one intermediate attempt traffic was already restored but the command still exited non-zero because act-as was missing. | Grant the backend GitHub deploy SA Artifact Registry Reader on `stds-staging-backend` and `roles/iam.serviceAccountUser` narrowly on `stds-cloud-run-core-staging`. Then rerun the rollback drill and verify the rollback step exits successfully before the expected final fail step. |
