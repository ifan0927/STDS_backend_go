...
| BR-27 | 正常退租結算 v1 遇到 `pending_payment`、`overdue`、`pending_meter` 帳單時不得 finalize；由 preview 回傳 blockers，frontend 不可自行把未結帳單折入結算 | 執行 checkout settlement preview/finalize 時 | 有 blocker 時 finalize 拒絕 | 強制終止流程另走 force termination |
